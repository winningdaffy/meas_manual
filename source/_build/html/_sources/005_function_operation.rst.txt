...
点击 :numref:`tool_bar_tab` 中的 **文件->选项** 按钮，如 :numref:`app_option` 所示：
...
在 **软件选项** 中包括了软件所有的全局设置，包括以下几个部分：

    - 设备驱动插件：列出了所有支持的设备驱动，可以根据需要勾选，如果不勾选则软件不会加载相应的设备驱动；如果某个设备驱动为灰色并且不可勾选，则表示驱动版本过低，需要升级
    - 分析算法插件：列出了所有支持分析算法
    - 搜索路径：配置文件及脚本的搜索路径，默认为试验项目路径和应用程序路径
    - 全局变量：可以定义全局变量，在软件中使用
